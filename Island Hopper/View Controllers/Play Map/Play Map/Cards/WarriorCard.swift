...
class WarriorCard: Card {
    ...
    override func setPlacingBlock(){
        self.startedPlacingBlock = { location in
            ...
            let entity = WarriorEntity(position: cord, textureCords: [16, 512], width: 16, height: 16, id: .Warrior)
...
//            self.controller.entityController.addEntity(entity: self.entityBeingPlaced!)
            guard let entity = self.entityBeingPlaced else {return}
            self.controller.orderQueue.addEntity(entity: entity)
